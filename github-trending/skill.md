...
description: >
  ...
  用户选择后生成 content.json，调用 pipeline-orchestrator 全流程出视频。
triggers:
  - 推荐 GitHub 项目
  - GitHub trending
  - 今天有什么好项目
...
> **网络依赖**：本 skill 需要大量外网访问（`gh search`、`gh api`、`curl`）。运行前确保已加载 `proxy/skill.md` 配置代理。

你是 GitHub 宝藏项目发现引擎。搜索 → 评分 → 用户选择 → 生成 content.json → 调用 pipeline-orchestrator。
...
### A1. 读去重记录
检查 `repo-analyzer/content/YYYY-repos.md`（如果存在）。已推荐的仓库不再出现。
...
```bash
gh search repos --updated ">=YYYY-MM-DD" --sort stars --limit 30
```
...
# 过去 7 天 Star 增量（通过 Star History API 或对比）
...
| 🚀 **Star 增速** | 过去 7 天增量 vs 总 Star 数 | 日均增量/总量 >2%:5, >1%:4, >0.5%:3, >0.1%:2, 停滞:1 |
...
| 👤 **作者背书** | owner followers + org 属性 | >10k followers 或 Apache/CNCF/Google/Microsoft 等:5, >1k followers:4, >100 followers:3, 普通:2, 无头像:1 |
...
| 🔥 **话题热度** | AI/Agent/MCP/Rust/工具链等当前热点 |
...
## Phase B：生成内容（委托 RepoAnalyzer）

用户选择后，调用 `repo-analyzer` skill 完成全部内容创作。

输出直接写入 pipeline 统一输出目录 `output/github/{YYYY-MM-DD-HHMM}/{repo_name}/`：

```
output/github/{YYYY-MM-DD-HHMM}/{repo_name}/
  ├── content.json              # 口播 + 素材绑定 + 源码分析 + 封面
  ├── material_manifest.json    # 素材清单
  └── materials/                # 下载的素材文件
```

RepoAnalyzer 自动完成：基础数据采集 → 目录扫描 → Top-15 源码评分选取 → 5 维深度分析 → content.json 生成（口播/封面/文案/源码洞察，质量铁律参见 repo-analyzer skill）。

更新去重记录：`repo-analyzer/content/YYYY-repos.md` 追加一行 `- owner/repo`。

---

## Phase C：调用 pipeline-orchestrator

```
调用 pipeline-orchestrator skill，传入：
  CONTENT_JSON    = repo-analyzer/content/YYYY-MM-DD/HHmm-{repo}-content.json
  REPO_URL        = {html_url}
  TOTAL_DURATION  = 根据 script.total_duration_est 估算（20-180 秒）
  EXTRA_URLS      = {homepage}（如果有）
```

Pipeline-orchestrator 接管后续全部流程 → final.mp4。
