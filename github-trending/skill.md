...
你是 GitHub 宝藏项目发现引擎。搜索 → 评分 → 用户选 → 生成 content.json → 调 pipeline-orchestrator。
...
检查 `repo-analyzer/content/YYYY-repos.md`（使用 `schema/dedup.py` 的 `DedupDB.load()`）。已推荐的仓库不再出现。
...
## Phase B：生成内容（委托 repo-analyzer）

用户选择后，调用 `repo-analyzer` skill 完成全部内容创作：

```
输入: REPO_URL + gh api 数据 + README + Top-15 源码分析
输出: repo-analyzer/content/YYYY-MM-DD/HHmm-{repo_name}-content.json
```

repo-analyzer 自动完成：基础数据采集→目录扫描→Top-15 源码评分选取→4 维深度分析→content.json 生成（口播/封面/文案/源码洞察，60-360 秒，质量铁律见 repo-analyzer skill）。

更新去重记录：`repo-analyzer/YYYY-repos.md` 追加一行 `- owner/repo`。
