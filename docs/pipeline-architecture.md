...
│  处理器 DAG: RepoAnalyzer→ScriptTimelineComposer                 │
...
┌──────────────────────────────────────────────────────────────────┐
│  ┌────────────────────┐    ┌──────────────────────┐              │
│  │ RepoAnalyzer       │    │ ScriptTimeline       │              │
│  │ repo-analyzer/     │───→│ Composer             │              │
│  │ GitHub URL →       │    │ timeline-composer/   │              │
│  │ ContentModel +     │    │ Content+Material →   │              │
│  │ material_discovery │    │ TimelineModel        │              │
│  │ + materials/       │    │ + VideoConfig        │              │
│  └────────────────────┘    └──────────┬───────────┘              │
│                                       │                          │
│                                       ▼                          │
│                              ┌──────────────────┐                │
...
### Processor: RepoAnalyzer（分析 + 素材发现 + 内容创作 — AI 决策）
...
3. **4 维深度分析**：
   ...
   - 代码质量与工程化规范
   - 性能、安全与扩展性
4. **README 素材发现与评估**：从 README 中识别架构图/benchmark/demo GIF/截图/代码段等，按 4 维度评分
5. **高价值素材下载**：评分 >= 3 的素材自动下载到本地 materials/ 目录
6. **内容创作**：生成口播脚本（8-20 segments）、封面提示词、发布文案、源码洞察

**输出文件**：
- `repo-analyzer/content/YYYY-MM-DD/HHmm-{repo_name}-content.json`（含 script/covers/publish_copy）
...
- 封面提示词中英文各一份，具体到视觉元素
- 去重记录写入 `repo-analyzer/content/YYYY-repos.md`（通过 `DedupDB().load().add(full_name).save()` 维护）
...
**输入**：`content.json` + `material_discovery.json` + 总时长
...
RepoAnalyzer 在素材发现阶段产出 `material_discovery.json`（含评分），同时在 materials/ 目录下产出 `material_manifest.json`（含已下载素材列表）供下游消费。
...
$OUTPUT_DIR/                          # repo-analyzer/content/YYYY-MM-DD/HHmm-{repo_name}
  -content.json                       # RepoAnalyzer 产出（含全部字段）
  material_discovery.json             # RepoAnalyzer 素材发现与评估清单
  materials/                          # RepoAnalyzer 下载的素材文件
  material_manifest.json              # RepoAnalyzer 素材 manifest（已下载素材列表）
  ...
  video_config.json                   # ScriptTimelineComposer 产出（覆盖 RepoAnalyzer 初始值）
...
| 路径 | 用途 |
|------|------|
| `repo-analyzer/skill.md` | RepoAnalyzer AI 操作指南（含素材发现 + 下载 + 脚本创作） |
| `repo-analyzer/schema/models.py` | import 重定向到 pipeline_contracts |
| `repo-analyzer/schema/dedup.py` | DedupDB 去重管理 |
| `repo-analyzer/scripts/recorder.mjs` | Playwright 录制（素材发现补充） |
| `repo-analyzer/scripts/allocate.py` | 素材验证编排 |
| `repo-analyzer/scripts/manifest_validator.py` | Manifest 校验 |
