"""
content-generator Pydantic 模型 — content.json 的运行时类型约束。

对应 content.schema.json 的 JSON Schema 定义，提供 model_dump() / model_validate()
等 Pydantic 能力。供 content-generator/skill.md 生成 content.json 时验证使用。
"""
...
class Covers(BaseModel):
    """封面提示词（3x4 + 16x9）"""
    _3x4: CoverVariant = Field(alias="3x4")
    _16x9: CoverVariant = Field(alias="16x9")
