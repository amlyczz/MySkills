...
from ...domain.analyzer.entities import ContentModel, DomainAnalysis, Script, ScriptSegment
from ...domain.blueprint.entities import (
...
class LLMBlueprintComposer(BlueprintComposer):
    ...
    async def _generate_skeleton(
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", load_prompt("visual_blueprint", "step1-skeleton-system.md")),
            ("user", load_prompt("visual_blueprint", "step1-skeleton-user.md")),
        ])
...
"""
        if segment:
            ...
            user_prompt += f"- Visual type: {segment.visual_type}\n"
            if segment.visual_params:
                user_prompt += f"- Visual params: {segment.visual_params}\n"

        user_prompt += f"\nProject: {content_title}\nPoints: {content_points}\n"
        user_prompt += "\nOutput a complete SceneConfig with rich elements and animations."

        prompt = ChatPromptTemplate.from_messages([
            ("system", load_prompt("visual_blueprint", "step2-elements-system.md")),
            ("user", "{scene_context}"),
        ])
    ...
    def _serialize_segments(script: Script) -> str:
        ...
        for i, seg in enumerate(script.segments):
            ...
            lines.append(
                f"Segment {i+1}: [{seg.visual_type}] ({seg.duration_est}s) "
                f"text='{text_preview}' params={seg.visual_params}"
            )
        return "\n".join(lines)
